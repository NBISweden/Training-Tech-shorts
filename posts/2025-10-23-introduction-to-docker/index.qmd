...
<details>
<summary>Show `r-plot-example/example_plot.r`</summary>
```{.r filename=r-plot-example/example_plot.r}
...
```{.dockerfile filename="custom-docker-example/Dockerfile"}
...
<details>
<summary>Show `ngs-analysis-example/ngs_analysis.sh`</summary>
```{.sh filename=ngs-analysis-example/ngs_analysis.sh}
#!/bin/bash
...
Often you can find the tool you want to run already packaged, and if it is not in Docker hub it could exist in other registries. See the [previous RSE walkthrough](http://localhost:6925/posts/2025-06-10-container-registry-intro/) on alternative registries for that.
