...
%%| fig-cap: "Two-terminal SSH tunneling setup"
flowchart TB
...
1. Connect from our Laptop to Dardel and get an allocation (Terminal 1).
2. SSH into the allocated worker node (Terminal 1).
3. Set up port forwarding from our Laptop through Dardel's login node (Terminal 2).
4. Start [Marimo](https://docs.marimo.io/) in headless mode on the worker node (Terminal 1).
5. Open the connection in our browser.
...
Note the node name from the output, then SSH into it:
...
:::{.callout-warning}
The `salloc` command in Terminal 1 **holds** the allocation. If you log out of Terminal 1 or the
connection drops, the allocation is released and the worker node becomes unavailable. Keep this
session open for the entire duration of your work.
:::
...
Where `-L` enables port forwarding, the first `8080` is the local port, and `<node_id>:8080` is the `host:hostport`.
If you only want to forward from the login node (e.g., for a service running there), use `-L 8080:localhost:8080` instead.
...
Back in **Terminal 1** (your interactive session on the worker node), let's make an
...
The `--host` listens on all interfaces, and `--port` is set to 8080.
...
In this case, select another port and try again, for example, changing ports to 8081.
:::
...
1. Connect from our Laptop to Pelle and request an interactive allocation (Terminal 1).
2. Set up port forwarding from our Laptop through Pelle's login node (Terminal 2).
3. Start [Marimo](https://docs.marimo.io/) in headless mode on the compute node (Terminal 1).
4. Open the connection in our browser.
...
Then request an interactive allocation. Unlike Dardel, the `interactive` command automatically places you on the allocated compute node — no manual `ssh` to the node is needed.

```{.bash}
interactive -A <project_id> -n 1 -t 1:00:00
```

Once resources are allocated, note the node name from the output, e.g.:
...
```{.bash}
ssh -L 8080:<node_id>:8080 pelle
```

:::{.callout-important}
After running this command, you will be on the **login node**, not the compute node. This is expected — the login node acts as a relay for the tunnel. Do **not** run your server here; that belongs in Terminal 1 on the compute node.
:::
...
:::{.callout-caution}
It's easy to accidentally run commands in the wrong location.
Use your prompt to identify where you are, e.g. `username@pelle1` means you're on the login node, and
`username@p115` means you're on the allocated worker node.
:::

#### Run Marimo using Pixi

Back in **Terminal 1** (your interactive session on the compute node), set up an environment using [Pixi](https://pixi.sh).
