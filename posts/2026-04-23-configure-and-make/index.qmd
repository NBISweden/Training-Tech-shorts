...
my_program: main.cpp engine.cpp utils.cpp # <1> <2>
	# Compile all source files and output the executable with -o flag
	g++ main.cpp engine.cpp utils.cpp -o my_program # <3> <4>
...
2. `make my_program` (or `make` since it's the first rule) only runs if `my_program` doesn't exist, or the source files change.
3. The recipe is indented using a {{< kbd Tab >}} character ( leading spaces cause an error )
4. Each line runs in it's own subshell ( it doesn't know what's run before - like a directory change ).
5. The file `clean` doesn't exist, so the command `make clean` always runs the recipe.
