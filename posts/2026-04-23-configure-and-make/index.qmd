...
my_program: main.cpp engine.cpp utils.cpp # <1>
...
1. The executable `my_program` (target) depends on these three source files.<br>
\- `make my_program` (or `make` since it's the first rule) only runs if `my_program` doesn't exist or the source files change.<br>
\- The recipe is indented using a {{< kbd Tab >}} character (leading spaces cause an error).<br>
