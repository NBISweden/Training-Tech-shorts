...
The real issue with this is that if you use keyboard shortcuts or the Copilot icon in the bottom-right corner to toggle completion, it does not apply only to the file or file type you are editing and it will not reset when you restart VSCode. The shortcuts will change the list in `GitHub › Copilot: Enable` and the changes will be persistent. In some other editors, the list defined in the settings is the default values and using shortcuts to modify behaviour is only used for the specific file you are editing and only until you restart the editor.
