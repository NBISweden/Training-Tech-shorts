...
To use the installed packages, one would need to set the environment variable `R_LIBS_USER` to this path, and invoke R. Alternatively, once in R one can also use `.libPaths()` to add the library path. Another otpion would be to define the environment in the `.Rprofile` file which lives in the user's home directory. The user must keep this in mind if switching between different R versions and library paths to make the necessary edits. The `usethis` package contains a helper function to edit this file.

```{.r}
## edit the .Rprofile file
usethis::edit_r_profile(scope = "user")

## add the following line to the opened .Rprofile file and save
#.libPaths(c("~/Library/R/arm64/4.5-Bioc-3.21/library", .libPaths()))
```
